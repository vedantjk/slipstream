...
TEST_F(ParserTest, ConvertsPricesExactly) {
  ...
  EXPECT_EQ(expectQuote("09:30:00.000,Q,SYNTH1,101.23,1,34.19,1,,,").bid_price,
            1012300);
  EXPECT_EQ(expectQuote("09:30:00.000,Q,SYNTH1,101.23,1,34.19,1,,,").ask_price,
            341900);

  // No fractional part, one decimal, four decimals, high-priced symbol.
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH5,,,,,1250,3,B").trade_price,
            12500000);
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH4,,,,,34.2,3,B").trade_price,
            342000);
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH4,,,,,0.0001,3,B").trade_price, 1);
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH5,,,,,1249.95,3,S").trade_price,
            12499500);
}
...
TEST_F(ParserTest, PassesNegativeAndZeroPricesThrough) {
  ...
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH1,,,,,-87.37,3,B").trade_price,
            -873700);
  EXPECT_EQ(expectTrade("09:30:00.000,T,SYNTH1,,,,,0,3,B").trade_price, 0);

  const QuoteData crossed =
      expectQuote("09:30:00.000,Q,SYNTH1,101.30,1,101.20,1,,,");
...
TEST_F(ParserTest, AcceptsQuantityAtUint32Max) {
  EXPECT_EQ(
      expectTrade("09:30:00.000,T,SYNTH1,,,,,1.00,4294967295,B").trade_qty,
      4294967295U);
...
TEST_F(ParserTest, RejectsNegativeQuantity) {
  // stoull("-1") does not throw, it wraps; the parser must catch the sign
  // itself.
...
TEST_F(ParserTest, AcceptsSymbolThatExactlyFillsWireField) {
  EXPECT_EQ(
      expectQuote("09:30:00.000,Q,ABCDEFGHIJKL,87.37,50,87.40,60,,,").symbol,
      "ABCDEFGHIJKL");
  EXPECT_EQ(expectTrade("09:30:00.000,T,ABCDEFGHIJKL,,,,,87.37,50,B").symbol,
            "ABCDEFGHIJKL");
}
...
TEST_F(ParserTest, RejectsWrongColumnCount) {
  ...
  expectRejected(
      "09:30:00.190,T,SYNTH2,,,,,248.53,65,B,");  // trade, 11 columns
}
...
TEST_F(ParserTest, RejectsRowThatIsNeitherQuoteNorTrade) {
  ...
  expectRejected(
      "09:30:00.003,q,SYNTH3,87.37,55,87.49,50,,,");  // case sensitive
  expectRejected("09:30:00.190,t,SYNTH2,,,,,248.53,65,B");
  expectRejected(
      "09:30:00.003,Quote,SYNTH3,87.37,55,87.49,50,,,");  // must be exactly "Q"
...
TEST_F(ParserTest, RejectsHeaderAndCommentLines) {
  // parseCSV skips these before calling parse(); parse() must still refuse
  // them.
  expectRejected(
...
TEST_F(ParserTest, RejectsQuoteWithNonNumericPrice) {
  ...
  expectRejected(
      "09:30:00.003,Q,SYNTH3,87.37abc,55,87.49,50,,,");  // trailing junk
...
TEST_F(ParserTest, RejectsTradeWithInvalidAggressor) {
  ...
  expectRejected(
      "09:30:00.190,T,SYNTH2,,,,,248.53,65,BUY");  // must be exactly one char
...
class ParseCsvTest : public ::testing::Test {
 protected:
  static std::string writeTempCsv(const std::string& name,
...
TEST_F(ParseCsvTest, SkipsPreambleHeaderAndBlankLines) {
  const std::string path = writeTempCsv(
      "slipstream_preamble.csv",
      "# Sample Market Data CSV\n"
      "# Format: Timestamp,Type,Symbol,BidPrice,BidQty,AskPrice,AskQty (for "
      "quotes)\n"
      "#\n"
      "\n"
      "Timestamp,Type,Symbol,BidPrice,BidQty,AskPrice,AskQty,Price,Qty,"
...
TEST_F(ParseCsvTest, CountsInvalidRowsAndKeepsValidOnesInOrder) {
  const std::string path =
      writeTempCsv("slipstream_mixed.csv",
                   "Timestamp,Type,Symbol,BidPrice,BidQty,AskPrice,AskQty,"
                   "Price,Qty,Aggressor\n"
                   "09:30:00.003,Q,SYNTH3,87.37,55,87.49,50,,,\n"
...
TEST_F(ParserTest, RejectsInvalidTimestampFormat) {
  expectRejected("9:30:00.003,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected("09-30-00.003,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected("09:30:00:003,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected("09:30:00.03,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected(" 09:30:00.003,Q,SYNTH1,87.37,50,87.40,60,,,");
}

TEST_F(ParserTest, RejectsTimestampOutsideValidTime) {
  expectRejected("24:00:00.000,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected("09:60:00.000,Q,SYNTH1,87.37,50,87.40,60,,,");
  expectRejected("09:30:60.000,Q,SYNTH1,87.37,50,87.40,60,,,");
}
