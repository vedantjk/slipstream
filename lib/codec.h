...
inline std::size_t encode(const QuoteData& data, std::span<std::byte> out) {
...
inline std::size_t encode(const TradeData& data, std::span<std::byte> out) {
  constexpr std::size_t encoded_size =
...
inline std::string readSymbol(const char (&symbol)[wire::kSymbolLen]) {
